...
def search_awards():
    ...
    query = """ SELECT p.name, mp.name, a.award_year, COUNT(*) as award_count
    FROM Award a
...
def find_youngest_oldest_actors():
    ...
    query = """ WITH ActorAwards AS (
    SELECT p.name, a.award_year, p.dob, 
	...
	SELECT name, age 
	FROM ActorAwards
	WHERE age = (SELECT MIN(age) FROM ActorAwards)
	   OR age = (SELECT MAX(age) FROM ActorAwards);
    """
...
def search_producers():
    ...
    WHERE m.boxoffice_collection >= %s AND mp.budget <= %s;
    """
...
def actors_marvel_warner():
    ...
    query = """ SELECT DISTINCT p.name, mp1.name
	FROM Role r1
	...
	WHERE (mp1.production = 'Marvel'
	AND mp2.production = 'Warner Bros') OR (mp2.production = 'Marvel'
	AND mp1.production = 'Warner Bros')AND r2.role_name = 'Actor';
    """
...
def top_5_movies_people_roles():
    ...
    query = """ SELECT mp.name, COUNT(DISTINCT p.id), COUNT(DISTINCT r.role_name)
    FROM Role r
    JOIN MotionPicture mp ON r.mpid = mp.id
    JOIN People p ON r.pid = p.id
    GROUP BY r.mpid
...
def actors_with_common_birthday():
    ...
    query = """SELECT DISTINCT p1.name AS actor_1, p2.name AS actor_2, p1.dob AS common_birthday
	FROM People p1
